...
class CurrencyConverter {
 private:
  void parseJsonFile(const std::string &filename, std::vector<Currency> &currencies) {
  ...
  double convertCurrency(double amount, const Currency &fromCurrency, const Currency &toCurrency) {
    return amount * (toCurrency.rate / fromCurrency.rate);
  }

 public:
  double convert(double amountToConvert, int fromCurrencyOrder, int toCurrencyOrder) {
...
class MoneySimulator {
 private:
  ...
  bool stringExistsInArray(const std::string &str, const std::string arr[], size_t size) { return std::find(arr, arr + size, str) != arr + size; }
 ...
 public:
  ...
  void gamble() {
    ...
    do {
      ...
      cin >> chosenColor;
      colorChosenIsValid = stringExistsInArray(chosenColor, colors, 4);
  ...
  void addMoney(double amount) { moneyAmount += amount; }

  void reduceMoney(double amount) { moneyAmount -= amount; }
